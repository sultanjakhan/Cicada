...
import { renderDayStartMarker } from './calendar-day-start.js';
  ...
  function tasksPanel(root, shell, options, state) {
    const items = (options.taskRecords || []).filter(record => record.source_type === 'note' && !record.readonly && !record.archived);
    const today = options.today || iso(new Date());
    const group = record => isCompleted(record) ? 5 : record.is_active ? 0 : !record.date ? 4 : record.date < today ? 1 : record.date === today ? 2 : 3;
    const groups = ['В работе', 'Просроченные', 'Сегодня', 'Позже', 'Без даты', 'Завершённые'];
    const tools = el('div', 'calv-view-tools');
    const aside = el('aside', 'calv-tasks-panel'); aside.id = `${state.id}-tasks`; aside.dataset.tasksPanel = '';
    const heading = el('h3', '', 'Задачи'); heading.id = `${state.id}-tasks-title`; heading.tabIndex = -1;
    heading.dataset.calendarControl = 'tasks-heading';
    aside.setAttribute('aria-labelledby', heading.id);
    const toggle = button('calv-panel-toggle', 'Задачи', () => {
    ...
    close.setAttribute('aria-label', 'Закрыть список задач');
    ...
    if (options.onCreateTask) {
      const create = button('calv-task-create', '+ Задача', () => options.onCreateTask(state.taskFilter === 'undated' ? null : options.date));
      create.dataset.taskCreate = ''; create.dataset.calendarControl = 'task-create'; aside.append(create);
    }
    if (options.taskError) {
      ...
      const filters = el('div', 'calv-task-filters'); filters.setAttribute('role', 'group'); filters.setAttribute('aria-label', 'Какие задачи показать');
      ...
      const dateHint = el('p', 'calv-panel-hint');
      const list = el('div', 'calv-task-list'); list.dataset.taskList = '';
      list.addEventListener('scroll', () => { state.taskScroll = list.scrollTop; });
      const matchFilter = (record, filter) => filter === 'completed' ? isCompleted(record) : !isClosed(record) && (filter === 'day' ? record.date === options.date : filter === 'undated' ? !record.date : true);
      for (const [id, title] of [['all', 'Активные'], ['day', 'На день'], ['undated', 'Без даты'], ['completed', 'Завершённые']]) {
        const filter = button('calv-task-filter', title, () => {
          state.taskFilter = id; state.taskScroll = 0; renderItems();
        });
        filter.dataset.taskFilter = id; filter.dataset.calendarControl = `task-filter-${id}`; filters.append(filter);
      }
      function renderItems() {
        const query = state.taskSearch.trim().toLocaleLowerCase('ru-RU');
        const visible = items.filter(record => matchFilter(record, state.taskFilter) && record.title.toLocaleLowerCase('ru-RU').includes(query))
          .sort((a, b) => group(a) - group(b) || (Number(b.priority) || 0) - (Number(a.priority) || 0) || (a.date || '9999').localeCompare(b.date || '9999') || a.title.localeCompare(b.title, 'ru'));
        filters.querySelectorAll('[data-task-filter]').forEach(filter => filter.setAttribute('aria-pressed', String(filter.dataset.taskFilter === state.taskFilter)));
        dateHint.textContent = state.taskFilter === 'day' ? label(options.date, { weekday: 'long', day: 'numeric', month: 'long' }) : '';
        dateHint.hidden = state.taskFilter !== 'day';
        let lastGroup = null;
        list.replaceChildren(...visible.flatMap(record => {
          const card = recordButton(record, options, 'calv-task-record');
          const meta = card.querySelector('.calv-record-meta');
          meta.textContent = [record.date && label(record.date), record.status !== groups[group(record)] && record.status !== 'Запланировано' && record.status].filter(Boolean).join(' · ');
          meta.hidden = !meta.textContent;
          const currentGroup = group(record);
          if (currentGroup === lastGroup) return [card];
          lastGroup = currentGroup;
          return [el('h4', 'calv-task-group', groups[currentGroup]), card];
        }));
        if (!visible.length) list.append(el('p', 'calv-empty', query ? 'По запросу задач нет.' : state.taskFilter === 'day' ? 'На этот день задач нет.' : state.taskFilter === 'undated' ? 'Задач без даты нет.' : state.taskFilter === 'completed' ? 'Завершённых задач нет.' : 'Незавершённых задач нет.'));
      ...
      aside.append(filters, search, dateHint, list); renderItems();
    }
  ...
  function render(root, options) {
    ...
    if (!viewStates.has(root)) viewStates.set(root, { id: `calv-${++viewSequence}`, tasksOpen: savedPanelOpen, taskFilter: 'all', taskSearch: '', taskScroll: 0, historyOpen: {}, untimedExpanded: {}, timeFoldsExpanded: {}, slotFocus: {} });
