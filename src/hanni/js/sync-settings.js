...
export function mountSyncSettings(element, { invoke, setPending = () => {} }) {
  ...
  function render() {
    ...
    if (statusError && (error.hidden || errorFromStatus)) { error.textContent = errors[statusError] || 'Последний обмен завершился ошибкой. Локальные изменения сохранены.'; error.hidden = false; errorFromStatus = true; }
