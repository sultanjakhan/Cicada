export function mountSyncSettings(element, { invoke, setPending = () => {} }) {
  ...
  let status = null, busy = false, disposed = false, dirty = false, revision = 0, errorFromStatus = false;
  ...
  function render() {
    ...
    const errors = { content_sync_network_unavailable:'Сеть недоступна. Повторим обмен после подключения.', content_sync_http_401:'Код подключения больше не принят сервером.', content_sync_http_403:'Устройство не имеет доступа к обмену.', content_sync_http_429:'Сервер просит повторить попытку позже.', content_sync_http_507:'На сервере не хватает места для изменений.', mvp_sync_pairing_changed:'Подключение изменилось. Повтори обмен с текущим подключением.', mvp_sync_invalid_config:'Проверь код подключения.', mvp_sync_backup_failed:'Не удалось создать резервную копию перед включением синхронизации.' };
    ...
    enabled.disabled = busy || !status;
    q('save').disabled = busy || !status || !dirty;
    q('cancel').disabled = busy || !dirty;
    q('now').disabled = busy || !status?.configured || !status.enabled || status.running === true;
    q('retry').hidden = !!status;
    code.disabled = busy; q('reveal').disabled = busy;
  }
  ...
  async function perform(save) {
    if (busy || disposed || !status) return;
    let configJson;
    ...
    busy = true; ++revision; error.hidden = true; errorFromStatus = false; setPending(true); render();
    try {
    ...
    } finally { configJson = undefined; busy = false; if (!disposed) { setPending(false); render(); } }
  }
  ...
  void refresh();
  return () => { disposed = true; ++revision; clearCode(); window.removeEventListener('hanni:sync-status', onStatus); window.removeEventListener('online', render); window.removeEventListener('offline', render); };
}
