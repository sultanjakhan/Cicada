...
export function mountSyncSettings(element, { invoke, setPending = () => {} }) {
  ...
  const credentialError = 'Нет доступа к сохранённому ключу синхронизации. Локальные данные сохранены. После восстановления доступа к хранилищу ключей перезапусти Cicada.';
  ...
  function render() {
    ...
    if (statusError && (error.hidden || errorFromStatus)) { error.textContent = credentialUnavailable(statusError) ? credentialError : errors[statusError] || 'Последний обмен завершился ошибкой. Локальные изменения сохранены.'; error.hidden = false; errorFromStatus = true; }
  ...
  async function perform(save) {
    ...
    try {
      ...
      if (!disposed) { errorFromStatus = false; error.textContent = credentialUnavailable(typeof cause === 'string' ? cause : cause?.message) ? credentialError : save ? 'Не удалось сохранить подключение. Проверь код и повтори попытку.' : 'Не удалось завершить обмен. Локальные изменения сохранены; повтори попытку при доступной сети.'; error.hidden = false; }
