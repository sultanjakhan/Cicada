...
export async function mountGoalDevelopment(element, { invoke, goal, onCreateTask } = {}) {
  ...
  function render() {
    ...
    const matched = ext.skills.filter(skill => (!stageSkillIds || stageSkillIds.has(skill.id)) && (!skillSearch || `${skill.title} ${skill.topic}`.toLocaleLowerCase('ru').includes(skillSearch))); const skills = [['hard','Хард'],['soft','Софт']].map(([group,label]) => { const grouped = matched.filter(skill => skill.group === group); if (!grouped.length) return ''; const expanded = Boolean(active || skillSearch); return `<details class="dev-group" ${expanded ? 'open' : ''}><summary><strong>${label}</strong><span>${grouped.filter(skillProgress).length} / ${grouped.length}</span></summary>${topicList(grouped).map(topic => `<details class="dev-topic" ${expanded ? 'open' : ''}><summary><strong>${escapeHtml(topic)}</strong><span>${grouped.filter(skill => skill.topic === topic).filter(skillProgress).length} / ${grouped.filter(skill => skill.topic === topic).length}</span></summary>${grouped.filter(skill => skill.topic === topic).map(skill => `<article class="dev-skill"><button type="button" data-dev-skill="${escapeHtml(skill.id)}">${escapeHtml(skill.title)}</button><span>${skill.evidence ? 'Подтверждено' : 'Без подтверждения'}</span>${skill.description || skill.practice ? `<p class="dev-skill-detail">${skill.description ? escapeHtml(skill.description) : ''}${skill.description && skill.practice ? ' · ' : ''}${skill.practice ? `Практика: ${escapeHtml(skill.practice)}` : ''}</p>` : ''}<div><button type="button" data-dev-evidence="${escapeHtml(skill.id)}">${skill.evidence ? 'Изменить результат' : 'Подтвердить'}</button><button type="button" data-dev-task="${escapeHtml(skill.id)}">Задача</button><button type="button" data-dev-remove="${escapeHtml(skill.id)}">Исключить</button></div></article>`).join('')}</details>`).join('')}</details>`; }).join('');
    const stageFilter = active ? `Этап: ${escapeHtml(active.title)}` : 'Все навыки';
    element.innerHTML = `<section class="calendar-development" aria-label="Развитие цели"><header><div><p class="dev-eyebrow">Развитие цели</p><h2>${escapeHtml(goal.title || 'Цель')}</h2><span class="dev-total-progress">${totalPercent == null ? 'Навыков пока нет' : `${totalPercent}% · ${totalDone} из ${ext.skills.length} подтверждено`}</span></div><div><button type="button" data-dev-stage-filter aria-haspopup="dialog">${stageFilter}</button><button type="button" data-dev-import>Импорт JSON</button><button type="button" data-dev-add>Добавить навык</button></div></header>${error ? `<p class="dev-error" role="alert">${escapeHtml(error)}</p>` : ''}<section class="dev-focus"><div><span>${active ? `Навык в этапе «${escapeHtml(active.title)}»` : 'Сейчас развиваю'}</span><strong>${focus ? `${escapeHtml(focus.topic)} · ${escapeHtml(focus.title)}` : active && !active.skillIds.length ? 'В этапе пока нет навыков' : 'Не выбрано'}</strong></div><button type="button" data-dev-focus>${focus ? 'Сменить или очистить' : 'Выбрать навык'}</button></section><section class="dev-stages"><header><h3>Этапы</h3><div>${active ? '<button type="button" data-dev-stage-clear>Вся цель</button>' : ''}<button type="button" data-dev-stage-add>Новый этап</button></div></header>${stageCards || '<p class="dev-empty">Раздели цель на ближайший результат и нужные навыки.</p>'}</section><section class="dev-skills"><header><h3>Навыки</h3><input type="search" data-dev-skill-search placeholder="Найти SQL, API или навык" value="${escapeHtml(skillSearch)}"><span>${matched.length} из ${ext.skills.length}</span></header>${skills || '<p class="dev-empty">В выбранном этапе нет подходящих навыков.</p>'}</section></section>`;
