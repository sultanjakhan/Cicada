...
export async function startCalendarExecution(invoke, task, document) {
  ...
  return invoke('start_task_block',{sourceType:task.source_type,sourceId:String(task.source_id),failIfActive:true,completionDate:task.completion_date||task.date||localDay()});
}
